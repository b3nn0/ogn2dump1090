...
class Dump1090Writer:
    ...
    async def send_msg(self, address : int, lat : float, lon : float, altFt : float, speedKt : float,
            climbRateFtMin : float = 0, track : float = 0, registration : str ='', anon : bool = False, addrtype : int = 0):
        now = time.time()
        rcvts = now # todo
