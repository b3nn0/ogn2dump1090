...
class Dump1090Writer:
    ...
    async def msgsender(self):
        while True:
            try:
                reader, self.sbsWriter = await asyncio.open_connection(config.sbs_destination_host, config.sbs_destination_port)
                logging.info(f"SBS Out connection to {config.sbs_destination_host}:{config.sbs_destination_port} established")
                await self.sbsWriter.wait_closed()
                self.sbsWriter = None
    ...
    async def send_msg(self, msg):
        if self.sbsWriter is None or self.sbsWriter.is_closing():
            return
        now = time.time()
        rcvts = now # todo
